...
public class CmdUnban implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args){
		...
		String playername;
		if(args.length == 0){
			sender.sendMessage(ChatColor.RED + "[" + ChatColor.GOLD + "MistCore" + ChatColor.RED + "]" + ChatColor.AQUA + " This command wasn't used correctly. Use it as /unban <playername> <reason>");
			return true;
		}
		String reason = null;
		if(args.length == 1){
		...
		if(cmd.getName().equalsIgnoreCase("unban")){
			message.sendmessage(false, playername, "unban", moderatorname, null, reason.replaceAll("_", " "));
		}else{
			message.sendmessage(true, playername, "unban", moderatorname, null, reason.replaceAll("_", " "));
		}
		return true;
	}
